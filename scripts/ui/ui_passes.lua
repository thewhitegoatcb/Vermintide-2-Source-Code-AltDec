...
local function extract_button_data_from_text(ui_renderer, ui_style, input_text, prepared_text)
	...
	if INPUT_ACTIONS[1] then
		...
		for i = 1, #INPUT_ACTIONS do
			...
			local replacement_width = UIRenderer.text_size(ui_renderer, "\xC2\xBD", font_material, font_size)
			local test_width = UIRenderer.text_size(ui_renderer, "\xC2\xBD\xC2\xA0", font_material, font_size)
			...
			if gamepad_active then
				...
				FINAL_REPLACEMENT_STR_LIST[i] = string.rep("\xC2\xA0", final_replacement_str_iterator)

				local replacement_str_iterator = math.ceil(button_width / replacement_width) + 1

				REPLACEMENT_STR_LIST[i] = string.rep("\xC2\xBD", replacement_str_iterator)
			else
